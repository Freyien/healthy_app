...
class ReminderStartInput extends StatelessWidget {
  const ReminderStartInput({super.key, required this.enable});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<WaterReminderBloc, WaterReminderState>(
      buildWhen: (p, c) => p.waterReminder.start != c.waterReminder.start,
      builder: (context, state) {
        return InputText(
          ...
          readOnly: true,
          enabled: enable,
          initialValue: state.waterReminder.start.format('HH:mm'),
          onTap: () {
            DatePicker.showDatePicker(
              context,
              locale: DateTimePickerLocale.es,
              pickerMode: DateTimePickerMode.time,
              initialDateTime: state.waterReminder.start,
